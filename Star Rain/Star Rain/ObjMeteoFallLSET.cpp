...
ObjMeteoFallLSET::ObjMeteoFallLSET(float x, float y)
{
	m_px = x;
	m_py = y;
}
...
void ObjMeteoFallLSET::Action()
{
	...
	if (Fall_f == true)
	{
		...
		if (m_hit_down == true)
		{
			m_speed_power_y = +0.00f;  //隕石落下速度y
			m_speed_power_x = -0.00f;	 //通常速度
		}
		if (m_hit_left == true)
		{
			m_speed_power_y = +0.00f;  //隕石落下速度y
